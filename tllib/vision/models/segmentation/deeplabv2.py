...
class Bottleneck(nn.Cell):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None):
        ...
        for i in self.bn1.trainable_params():
        ...
        for i in self.bn2.trainable_params():
            i.requires_grad = False

        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(planes * 4, affine=affine_par)
        for i in self.bn3.trainable_params():
            i.requires_grad = False
...
class ResNet(nn.Cell):
    def __init__(self, block, layers):
        ...
        for m in self.cells():
            if isinstance(m, nn.Conv2d):
                ...
                m.gamma.set_data(ops.fill(mindspore.float32, m.gamma.shape, 1))
                m.beta.set_data(ops.zeros_like(m.beta))
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        for i in downsample[1].trainable_params():
            i.requires_grad = False
        layers = []
...
class Deeplab(nn.Cell):
    ...
    def get_10x_lr_params(self):
        ...
        """
        for param in self.classifier.trainable_params():
            yield param
