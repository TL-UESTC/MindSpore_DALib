...
class JointsMSELoss(nn.Cell):
    ...
    def construct(self, output, target, target_weight=None):
...
class JointsKLLoss(nn.Cell):
    ...
    def construct(self, output, target, target_weight=None):
        B, K, _, _ = output.shape
        heatmaps_pred = output.reshape((B, K, -1))
        heatmaps_pred = ops.log_softmax(heatmaps_pred, axis=-1)
        heatmaps_gt = target.reshape((B, K, -1))
        heatmaps_gt = heatmaps_gt + self.epsilon
        heatmaps_gt = heatmaps_gt / heatmaps_gt.sum(axis=-1, keepdims=True)
        loss = self.criterion(heatmaps_pred, heatmaps_gt).sum(axis=-1)
        if target_weight is not None:
