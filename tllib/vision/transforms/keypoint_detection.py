...
# import msadapter.torchvision.transforms.transforms as T
import mindspore.dataset.vision as V
import mindspore.dataset.vision.py_transforms_util as U
import numbers
import random
import math
import warnings
...
ToTensor = wrapper(V.ToTensor)
Normalize = wrapper(V.Normalize)
ColorJitter = wrapper(V.RandomColorAdjust)
...
def resize(image: Image.Image, size: int, interpolation=Image.BILINEAR,
    ...
    image =U.resize(image, size, interpolation)
...
def crop(image: Image.Image, top, left, height, width, keypoint2d: np.ndarray):
    image = U.crop(image, top, left, height, width)
    keypoint2d = np.copy(keypoint2d)
    keypoint2d[:, 0] -= left
    keypoint2d[:, 1] -= top
...
class Resize(object):
    ...
    def __call__(self, image, keypoint2d: np.ndarray, intrinsic_matrix: np.ndarray, **kwargs):
        ...
        if 'depth' in kwargs:
            kwargs['depth'] = U.resize(kwargs['depth'], self.size)
        return image, kwargs
...
class CenterCrop(object):
    ...
    def __call__(self, image, keypoint2d, **kwargs):
        ...
        kwargs.update(keypoint2d=keypoint2d)
        if 'depth' in kwargs:
            kwargs['depth'] = U.center_crop(kwargs['depth'], self.size)
        return image, kwargs
...
class RandomRotation(object):
    ...
    def __call__(self, image, keypoint2d, **kwargs):
        ...
        kwargs.update(keypoint2d=keypoint2d)
        if 'depth' in kwargs:
            kwargs['depth'] = U.rotate(kwargs['depth'], angle)
        return image, kwargs
...
class RandomResizedCrop(object):
    ...
    def __call__(self, image, keypoint2d: np.ndarray, intrinsic_matrix: np.ndarray, **kwargs):
        ...
        kwargs.update(keypoint2d=keypoint2d, intrinsic_matrix=intrinsic_matrix)
        if 'depth' in kwargs:
            kwargs['depth'] = U.resize(U.crop(kwargs['depth'], i, j, h, w), self.size, self.interpolation,)
        return image, kwargs
