...
class Regressor(nn.Cell):
    ...
    def __init__(self, backbone: nn.Cell, num_factors: int, bottleneck: Optional[nn.Cell] = None,
                 bottleneck_dim=-1, head: Optional[nn.Cell] = None, finetune=True):
        ...
        if head is None:
            self.head = nn.SequentialCell(
                nn.Dense(self._features_dim, num_factors),
                nn.Sigmoid()
            )
        else:
            self.head = head
