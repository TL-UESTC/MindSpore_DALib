...
class LeastSquaresGenerativeAdversarialLoss(nn.Cell):
    """
    ...
    def construct(self, prediction, real=True):
        if real:
            label = ops.ones_like(prediction)
        else:
            label = ops.zeros_like(prediction)
        return self.mse_loss(prediction, label)


class VanillaGenerativeAdversarialLoss(nn.Cell):
    """
