...
class ResnetBlock(nn.Cell):
    ...
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        and implement skip connections in <construct> function.
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, has_bias=use_bias), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, has_bias=use_bias), norm_layer(dim)]

        return nn.SequentialCell(*conv_block)

    def construct(self, x):
        """construct function (with skip connections)"""
...
class ResnetGenerator(nn.Cell):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        model = [nn.ReflectionPad2d(3),
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, has_bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, has_bias=use_bias),
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        downconv = nn.Conv2d(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, has_bias=use_bias)
    ...
    def construct(self, x):
        if self.outermost:
            ...
            return ops.concat([x, self.model(x)], 1)
