...
class MinimumClassConfusionLoss(nn.Cell):
    r"""
    ...
    def construct(self, logits: mindspore.Tensor) -> mindspore.Tensor:
        batch_size, num_classes = logits.shape
        predictions = ops.softmax(logits / self.temperature, axis=1)  # batch_size x num_classes
        entropy_weight = entropy(predictions)
        entropy_weight = 1 + ops.exp(-entropy_weight)
        entropy_weight = (batch_size * entropy_weight / ops.sum(entropy_weight)).unsqueeze(dim=1)  # batch_size x 1
        class_confusion_matrix = ops.mm((predictions * entropy_weight).transpose(1, 0), predictions) # num_classes x num_classes
        class_confusion_matrix = class_confusion_matrix / ops.sum(class_confusion_matrix, dim=1)
        mcc_loss = (ops.sum(class_confusion_matrix) - ops.trace(class_confusion_matrix)) / num_classes
        return mcc_loss
