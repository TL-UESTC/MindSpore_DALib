...
class ImageClassifier(Classifier):
    ...
    def __init__(self, backbone: nn.Cell, num_classes: int, bottleneck_dim=1024, width=2048, **kwargs):
        ...
        self.pseudo_head = nn.SequentialCell(
            nn.Dense(self.features_dim, width),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Dense(width, self.num_classes)
        )
        self.grl_layer = WarmStartGradientReverseLayer(alpha=1.0, lo=0.0, hi=0.1, max_iters=1000, auto_step=False)
        self.adv_head = nn.SequentialCell(
            nn.Dense(self.features_dim, width),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Dense(width, self.num_classes)
    ...
    def get_parameters_own(self, base_lr=1.0):
        ...
        """
        params = [
            {"params": self.get_parameters(self.backbone), "lr": 0.1 * base_lr if self.finetune else 1.0 * base_lr},
            {"params": self.get_parameters(self.bottleneck), "lr": 1.0 * base_lr},
            {"params": self.get_parameters(self.head), "lr": 1.0 * base_lr},
            {"params": self.get_parameters(self.pseudo_head), "lr": 1.0 * base_lr},
            {"params": self.adv_head.get_parameters(), "lr": 1.0 * base_lr}
        ]
...
class WorstCaseEstimationLoss(nn.Cell):
    r"""
    ...
    def construct(self, y_l, y_l_adv, y_u, y_u_adv):
        _, prediction_l = y_l.max(axis=1)
        loss_l = self.eta_prime * ops.cross_entropy(y_l_adv, prediction_l)

        _, prediction_u = y_u.max(axis=1)
        loss_u = ops.nll_loss(shift_log(1. - ops.softmax(y_u_adv, dim=1)), prediction_u)
