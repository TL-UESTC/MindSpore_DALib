...
class ConfidenceBasedSelfTrainingLoss(nn.Cell):
    """
    ...
    def construct(self, y, y_target):
        confidence, pseudo_labels = ops.softmax(y_target, axis=1).max(axis=1)
        mask = (confidence > self.threshold).float()
        self_training_loss = (ops.cross_entropy(y, pseudo_labels, reduction='none') * mask).mean()
