...
class EMATeacher(object):
    ...
    def update(self):
        for teacher_param, param in zip(self.teacher.get_parameters(), self.model.get_parameters()):
            teacher_param.data = self.alpha * teacher_param + (1 - self.alpha) * param

    def __call__(self, x: mindspore.Tensor):
        return self.teacher(x)

    def train(self, mode: Optional[bool] = True):
        self.teacher.set_train(mode)
    ...
    def state_dict(self):
        state_dict = {}
        params = self.teacher.get_parameters()
        for param in params:
            name = param.name
    ...
    def load_state_dict(self, state_dict):
        mindspore.load_param_into_net(self.teacher, state_dict)

    @property
    def module(self):
        return self.teacher.cells
...
def update_bn(model, ema_model):
    ...
    for m2, m1 in zip(ema_model.cells_and_names(), model.cells_and_names()):
        if ('bn' in m2[0]) and ('bn' in m1[0]):
            bn2, bn1 = m2[1], m1[1]
            bn2.moving_mea = bn1.moving_mean
            bn2.moving_variance = bn1.moving_variance
