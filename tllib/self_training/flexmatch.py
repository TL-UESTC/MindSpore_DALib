...
class DynamicThresholdingModule(object):
    ...
    def __init__(self, threshold, warmup, mapping_func, num_classes, n_unlabeled_samples, device):
        ...
        self.net_outputs = ops.zeros(n_unlabeled_samples, dtype=mindspore.int32)
        self.net_outputs.fill(-1)
        self.device = device

    def get_threshold(self, pseudo_labels):
        """Calculate and return dynamic threshold"""
        pseudo_counter = Counter(self.net_outputs.asnumpy().tolist())
        if max(pseudo_counter.values()) == self.n_unlabeled_samples:
            ...
            status = ops.zeros(self.num_classes)
        else:
            ...
            status = mindspore.Tensor(status.asnumpy(), mindspore.float32)
