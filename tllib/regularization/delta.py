...
class L2Regularization(nn.Cell):
    ...
    def construct(self):
        ...
        for param in self.model.get_parameters():
            output += 0.5 * ops.norm(param) ** 2
        return output


class SPRegularization(nn.Cell):
    r"""
    ...
    Shape:
        - Output: scalar.
    """
    def __init__(self, source_model: nn.Cell, target_model: nn.Cell):
        super(SPRegularization, self).__init__()
        self.target_model = target_model
        self.source_weight = {}
        for name, param in source_model.named_parameters():
            self.source_weight[name] = param

    def construct(self):
        output = 0.0
        for name, param in self.target_model.named_parameters():
            output += 0.5 * ops.norm(param - self.source_weight[name]) ** 2
        return output


class BehavioralRegularization(nn.Cell):
    r"""
    ...
    def construct(self, layer_outputs_source, layer_outputs_target):
        output = 0.0
        for fm_src, fm_tgt in zip(layer_outputs_source.values(), layer_outputs_target.values()):
            output += 0.5 * (ops.norm(fm_tgt - fm_src) ** 2)
        return output


class AttentionBehavioralRegularization(nn.Cell):
    r"""
    ...
    def construct(self, layer_outputs_source, layer_outputs_target):
        ...
        for i, (fm_src, fm_tgt) in enumerate(zip(layer_outputs_source.values(), layer_outputs_target.values())):
            ...
            distance = ops.norm(fm_tgt - fm_src, 2, 2)
            distance = c * ops.mul(self.channel_attention[i], distance ** 2) / (h * w)
            output += 0.5 * ops.sum(distance)
