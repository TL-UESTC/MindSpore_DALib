...
class AutomaticUpdateClassWeightModule(object):
    ...
    def __init__(self, update_steps: int, data_loader: GeneratorDataset,
                 classifier: nn.Cell, num_classes: int,
                 device, temperature: Optional[float] = 0.1,
                 partial_classes_index: Optional[List[int]] = None):
        self.update_steps = update_steps
        self.data_loader = data_loader
        self.classifier = classifier
        self.class_weight_module = ClassWeightModule(temperature)
        self.class_weight = ops.ones(num_classes)
        self.num_steps = 0
    ...
    def get_class_weight_for_adversarial_loss(self, source_labels: mindspore.Tensor) -> Tuple[mindspore.Tensor, mindspore.Tensor]:
        """
        Outputs:
        ...
        class_weight_adv_target = ops.ones_like(class_weight_adv_source) * class_weight_adv_source.mean()
    ...
    def get_partial_classes_weight(self):
        ...
        return ops.mean(self.class_weight[self.partial_classes_index]), ops.mean(
            self.class_weight[self.non_partial_classes_index])


class ClassWeightModule(nn.Cell):
    r"""
    ...
    def construct(self, outputs: mindspore.Tensor):
        softmax_outputs = ops.softmax(outputs / self.temperature, axis=1)
        class_weight = ops.mean(softmax_outputs, axis=0)
        class_weight = class_weight / ops.max(class_weight)
        class_weight = class_weight.view(-1)
        return class_weight


def collect_classification_results(data_loader: GeneratorDataset, classifier: nn.Cell,
                                   device) -> mindspore.Tensor:
    """
    ...
    classifier.set_train(mode = False)
    all_outputs = []
    for i, (images, target) in enumerate(data_loader):
        output = classifier(images)
        all_outputs.append(output)
    classifier.set_train(training)
    return ops.concat(all_outputs, axis=0)
