...
class ForeverDataIterator:
    ...
    def __init__(self, data_loader: GeneratorDataset, device=None):
...
class RandomMultipleGallerySampler(Sampler):
    ...
    def __iter__(self):
        ...
        pid_idxes = ops.randperm(len(self.pid_list)).asnumpy().tolist()
        final_idxes = []
...
def concatenate(tensors):
    ...
    """
    if isinstance(tensors[0], mindspore.Tensor):
        return ops.cat(tensors, dim=0)
