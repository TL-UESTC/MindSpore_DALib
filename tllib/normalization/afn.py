...
class AdaptiveFeatureNorm(nn.Cell):
    r"""
    ...
    def construct(self, f: mindspore.Tensor) -> mindspore.Tensor:
        radius = f.norm(ord=2, dim=1)
        radius = radius + self.delta
        loss = ((f.norm(ord=2, dim=1) - radius) ** 2).mean()
        return loss
...
class Block(nn.Cell):
    ...
    def __init__(self, in_features: int, bottleneck_dim: Optional[int] = 1000, dropout_p: Optional[float] = 0.5):
        ...
        self.fc = nn.Dense(in_features, bottleneck_dim)
    ...
    def construct(self, x: mindspore.Tensor) -> mindspore.Tensor:
        ...
        f = self.dropout(f)
        if self.training:
            f.mul(math.sqrt(1 - self.dropout_p))
        return f
...
class ImageClassifier(ClassfierBase):
    ...
    def __init__(self, backbone: nn.Cell, num_classes: int, num_blocks: Optional[int] = 1,
        ...
        bottleneck = nn.SequentialCell(*layers)
        super(ImageClassifier, self).__init__(backbone, num_classes, bottleneck, bottleneck_dim, **kwargs)
        # init parameters for bottleneck and head
        for m in self.bottleneck.cells():
            if isinstance(m, nn.BatchNorm1d):
                m.weight.set_data(ops.normal(m.weight.shape, 1, 0.01))
    ...
    def get_parameters_own(self, base_lr=1.0) -> List[Dict]:
        params = [
            ...
            {"params": self.get_parameters(self.head), "momentum": 0.9},
        ]
        return params
