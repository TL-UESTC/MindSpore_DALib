...
class MultipleKernelMaximumMeanDiscrepancy(nn.Cell):
    ...
    Examples::
        ...
        >>> z_s, z_t = mindspore.ops.randn(batch_size, feature_dim), minspre.ops.randn(batch_size, feature_dim)
        >>> output = loss(z_s, z_t)
    """
        
    def __init__(self, kernels: Sequence[nn.Cell], linear: Optional[bool] = False):
    ...
    def construct(self, z_s: mindspore.Tensor, z_t: mindspore.Tensor) -> mindspore.Tensor:
        features = ops.concat([z_s, z_t], axis=0)
...
def _update_index_matrix(batch_size: int, index_matrix: Optional[mindspore.Tensor] = None,
                         linear: Optional[bool] = True) -> mindspore.Tensor:
    r"""
    ...
    if index_matrix is None or index_matrix.shape[0] != batch_size * 2:
        index_matrix = ops.zeros(2 * batch_size, 2 * batch_size)
        if linear:
