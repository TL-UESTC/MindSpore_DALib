...
class DomainAdversarialLoss(nn.Cell):
    r"""
    ...
    def __init__(self, domain_discriminator: nn.Module, reduction: Optional[str] = 'mean',
        ...
        self.bce = lambda input, target, weight: \
            ops.binary_cross_entropy(input, target, weight=weight, reduction=reduction)
        self.domain_discriminator_accuracy = None

    def construct(self, f_s: mindspore.Tensor, f_t: mindspore.Tensor,
                w_s: Optional[mindspore.Tensor] = None, w_t: Optional[mindspore.Tensor] = None) -> mindspore.Tensor:
        f = self.grl(ops.concat((f_s, f_t), axis=0))
        d = self.domain_discriminator(f)
        if self.sigmoid:
            d_s, d_t = d.chunk(2, axis=0)
            d_label_s = ops.ones((f_s.size(0), 1))
            d_label_t = ops.zeros((f_t.size(0), 1))
            self.domain_discriminator_accuracy = 0.5 * (
                        binary_accuracy(d_s, d_label_s) + binary_accuracy(d_t, d_label_t))

            if w_s is None:
                w_s = ops.ones_like(d_label_s)
            if w_t is None:
                w_t = ops.ones_like(d_label_t)
            return 0.5 * (
                ops.binary_cross_entropy(d_s, d_label_s, weight=w_s.view_as(d_s), reduction=self.reduction) +
                ops.binary_cross_entropy(d_t, d_label_t, weight=w_t.view_as(d_t), reduction=self.reduction)
            )
        else:
            d_label = ops.concat((
                ops.ones((f_s.size(0),)),
                ops.zeros((f_t.size(0),)),
            )).long()
            if w_s is None:
                w_s = ops.ones((f_s.size(0),))
            if w_t is None:
                w_t = ops.ones((f_t.size(0),))
            self.domain_discriminator_accuracy = accuracy(d, d_label)
            loss = ops.cross_entropy(d, d_label, reduction='none') * ops.concat([w_s, w_t], axis=0)
