...
from tllib.alignment.mk_mmd import _update_index_matrix
...
class JointMultipleKernelMaximumMeanDiscrepancy(nn.Module):
    ...
    def __init__(self, kernels: Sequence[Sequence[nn.Cell]], linear: Optional[bool] = True, thetas: Sequence[nn.Cell] = None):
    ...
    def construct(self, z_s: mindspore.Tensor, z_t: mindspore.Tensor) -> mindspore.Tensor:
        batch_size = int(z_s[0].size(0))
        self.index_matrix = _update_index_matrix(batch_size, self.index_matrix, self.linear).to(z_s[0])

        kernel_matrix = ops.ones_like(self.index_matrix)
        for layer_z_s, layer_z_t, layer_kernels, theta in zip(z_s, z_t, self.kernels, self.thetas):
            layer_features = ops.concat([layer_z_s, layer_z_t], axis=0)
...
class Theta(nn.Module):
    ...
    def __init__(self, dim: int):
        ...
        self.layer1 = nn.Dense(dim, dim)
        # nn.init.eye_(self.layer1.weight)
        # nn.init.zeros_(self.layer1.bias)
        self.layer1.weight.set_data(mindspore.Tensor(mindspore.numpy.eye(dim, dim)))
        self.layer1.bias.set_data(mindspore.Tensor(mindspore.numpy.zeros((10,))))
