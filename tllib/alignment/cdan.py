...
class ConditionalDomainAdversarialLoss(nn.Cell):
    ...
    def __init__(self, domain_discriminator: nn.Cell, entropy_conditioning: Optional[bool] = False,
        ...
        self.bce = lambda input, target, weight: ops.binary_cross_entropy(input, target, weight,
                                                                        reduction=reduction) if self.entropy_conditioning \
            else ops.binary_cross_entropy(input, target, reduction=reduction)
        self.domain_discriminator_accuracy = None

    def construct(self, g_s: mindspore.Tensor, f_s: mindspore.Tensor, g_t: mindspore.Tensor, f_t: mindspore.Tensor) -> mindspore.Tensor:
        f = ops.concat((f_s, f_t), axis=0)
        g = ops.concat((g_s, g_t), axis=0)
        g = ops.softmax(g, axis=1)
        h = self.grl(self.map(f, g))
        d = self.domain_discriminator(h)

        weight = 1.0 + ops.exp(-entropy(g))
        batch_size = f.shape[0]
        weight = weight / ops.sum(weight) * batch_size

        if self.sigmoid:
            d_label = ops.concat((
                ops.ones((g_s.size(0), 1)),
                ops.zeros((g_t.size(0), 1)),
            ))
            self.domain_discriminator_accuracy = binary_accuracy(d, d_label)
            if self.entropy_conditioning:
                return ops.binary_cross_entropy(d, d_label, weight.view_as(d), reduction=self.reduction)
            else:
                return ops.binary_cross_entropy(d, d_label, reduction=self.reduction)
        else:
            d_label = ops.concat((
                ops.ones((g_s.size(0), )),
                ops.zeros((g_t.size(0), )),
            )).long()
            d_label = mindspore.Tensor(d_label, dtype=mindspore.int32)
            self.domain_discriminator_accuracy = accuracy(d, d_label)
            if self.entropy_conditioning:
                raise NotImplementedError("entropy_conditioning")
            return ops.cross_entropy(d, d_label, reduction=self.reduction)


class RandomizedMultiLinearMap(nn.Cell):
    ...
    def __init__(self, features_dim: int, num_classes: int, output_dim: Optional[int] = 1024):
        ...
        self.Rf = ops.randn(features_dim, output_dim)
        self.Rg = ops.randn(num_classes, output_dim)
        self.output_dim = output_dim

    def construct(self, f: mindspore.Tensor, g: mindspore.Tensor) -> mindspore.Tensor:
        f = ops.mm(f, self.Rf)
        g = ops.mm(g, self.Rg)
        output = ops.mul(f, g) / np.sqrt(float(self.output_dim))
        return output


class MultiLinearMap(nn.Cell):
    """Multi linear map
    ...
    def construct(self, f: mindspore.Tensor, g: mindspore.Tensor) -> mindspore.Tensor:
        batch_size = f.size(0)
        output = ops.bmm(g.unsqueeze(2), f.unsqueeze(1))
