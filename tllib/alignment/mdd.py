...
class MarginDisparityDiscrepancy(nn.Cell):
    ...
    def construct(self, y_s: mindspore.Tensor, y_s_adv: mindspore.Tensor, y_t: mindspore.Tensor, y_t_adv: mindspore.Tensor,
                w_s: Optional[mindspore.Tensor] = None, w_t: Optional[mindspore.Tensor] = None) -> mindspore.Tensor:

        source_loss = -self.margin * self.source_disparity(y_s, y_s_adv)
        target_loss = self.target_disparity(y_t, y_t_adv)
        if w_s is None:
            w_s = ops.ones_like(source_loss)
        source_loss = source_loss * w_s
        if w_t is None:
            w_t = ops.ones_like(target_loss)
...
class ClassificationMarginDisparityDiscrepancy(MarginDisparityDiscrepancy):
    ...
    def __init__(self, margin: Optional[float] = 4, **kwargs):
        def source_discrepancy(y: mindspore.Tensor, y_adv: mindspore.Tensor):
            _, prediction = y.max(axis=1)
            return ops.cross_entropy(y_adv, prediction, reduction='none')

        def target_discrepancy(y: mindspore.Tensor, y_adv: mindspore.Tensor):
            _, prediction = y.max(axis=1)
            return -ops.nll_loss(shift_log(1. - ops.softmax(y_adv, dim=1)), prediction, reduction='none')
...
class RegressionMarginDisparityDiscrepancy(MarginDisparityDiscrepancy):
    ...
    def __init__(self, margin: Optional[float] = 1, loss_function=ops.l1_loss, **kwargs):
        def source_discrepancy(y: mindspore.Tensor, y_adv: mindspore.Tensor):
            return loss_function(y_adv, y, reduction='none')
...
def shift_log(x: mindspore.Tensor, offset: Optional[float] = 1e-6) -> mindspore.Tensor:
    r"""
    ...
    """
    return ops.log(ops.clamp(x + offset, max=1.))


class GeneralModule(nn.Cell):
    def __init__(self, backbone: nn.Cell, num_classes: int, bottleneck: nn.Cell,
                 head: nn.Cell, adv_head: nn.Cell, grl: Optional[WarmStartGradientReverseLayer] = None,
    ...
    def construct(self, x: mindspore.Tensor) -> Tuple[mindspore.Tensor, mindspore.Tensor]:
        """"""
    ...
    def get_parameters_own(self, base_lr=1.0) -> List[Dict]:
        ...
        params = [
            ...
            {"params": self.get_parameters(self.head), "lr": base_lr},
            {"params": self.get_parameters(self.adv_head), "lr": base_lr}
        ]
        return params
...
class ImageClassifier(GeneralModule):
    ...
    def __init__(self, backbone: nn.Cell, num_classes: int,
        ...
        weight = bottleneck[1].weight
        bias = bottleneck[1].bias
        weight.set_data(ops.normal(weight.shape, 0, 0.005)) 
        bias.set_data(ops.fill(mindspore.float32, bias.shape, 0.1))

        # The classifier head used for final predictions.
        head = nn.SequentialCell(
            nn.Dense(bottleneck_dim, width),
            nn.ReLU(),
            nn.Dropout(p = 0.5),
            nn.Dense(width, num_classes)
        )
        # The adversarial classifier head
        adv_head = nn.SequentialCell(
            nn.Dense(bottleneck_dim, width),
            nn.ReLU(),
            nn.Dropout(p = 0.5),
            nn.Dense(width, num_classes)
        ...
        for dep in range(2):
            head[dep * 3].weight.set_data(ops.normal(head[dep * 3].weight.shape, 0, 0.01)) 
            head[dep * 3].bias.set_data(ops.fill(mindspore.float32, head[dep * 3].bias.shape, 0.0))
            adv_head[dep * 3].weight.set_data(ops.normal(adv_head[dep * 3].weight.shape, 0, 0.01)) 
            adv_head[dep * 3].bias.set_data(ops.fill(mindspore.float32, adv_head[dep * 3].bias.shape, 0.0))
...
class ImageRegressor(GeneralModule):
    ...
    def __init__(self, backbone: nn.Cell, num_factors: int, bottleneck = None, head=None, adv_head=None,
        ...
        if head is None:
            head = nn.SequentialCell(
                ...
                nn.AdaptiveAvgPool2d(output_size=(1, 1)),
                nn.Flatten(),
                nn.Dense(width, num_factors),
                nn.Sigmoid()
            )
            for layer in head:
                if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Dense):
                    layer.weight.set_data(ops.normal(layer.weight.shape, 0, 0.01))
                    layer.bias.set_data(ops.fill(mindspore.float32, layer.bias.shape, 0))
