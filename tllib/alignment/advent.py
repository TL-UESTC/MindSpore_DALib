...
class Discriminator(nn.SequentialCell):
    """
...
def prob_2_entropy(prob):
    ...
    """
    n, c, h, w = prob.shape
    return -ops.mul(prob, ops.log2(prob + 1e-30)) / np.log2(c)


def bce_loss(y_pred, y_label):
    y_truth_tensor = ops.fill(mindspore.float32,y_pred.shape,y_label)   
    weight = mindspore.Tensor(np.ones_like(y_pred.shape[1]), mindspore.float32)
    pos_weight = mindspore.Tensor(np.ones_like(y_pred.shape[1]), mindspore.float32)
    return ops.binary_cross_entropy_with_logits(y_pred, y_truth_tensor,weight,pos_weight)


class DomainAdversarialEntropyLoss(nn.Cell):
    ...
    Args:
        domain_discriminator (mindspore.nn.Cell): A domain discriminator object, which predicts
    ...
    Examples::
        ...
        >>> y_s, y_t = ops.randn(B, C, H, W), ops.randn(B, C, H, W)
        >>> loss = 0.5 * (dann(y_s, "source") + dann(y_t, "target"))
    """
    def __init__(self, discriminator: nn.Cell):
        super(DomainAdversarialEntropyLoss, self).__init__()
        self.discriminator = discriminator

    def construct(self, logits, domain_label='source'):
        """
        """
        assert domain_label in ['source', 'target']
        probability = ops.softmax(logits, axis=1)
    ...
    def train(self, mode=True):
        ...
        """
        self.discriminator.set_train(mode)
        for param in self.discriminator.trainable_params():
