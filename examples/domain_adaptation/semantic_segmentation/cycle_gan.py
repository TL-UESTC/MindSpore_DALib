...
def main(args):
    ...
    netG_S2T = cyclegan.generator.__dict__[args.netG](ngf=args.ngf, norm=args.norm, use_dropout=False)
    netG_T2S = cyclegan.generator.__dict__[args.netG](ngf=args.ngf, norm=args.norm, use_dropout=False)
    netD_S = cyclegan.discriminator.__dict__[args.netD](ndf=args.ndf, norm=args.norm)
    netD_T = cyclegan.discriminator.__dict__[args.netD](ndf=args.ndf, norm=args.norm)
    ...
    optimizer_G = Adam(itertools.chain(netG_S2T.trainable_params(), netG_T2S.trainable_params()), lr=args.lr, betas=(args.beta1, 0.999))
    optimizer_D = Adam(itertools.chain(netD_S.trainable_params(), netD_T.trainable_params()), lr=args.lr, betas=(args.beta1, 0.999))
    ...
    if args.resume:
        ...
        mindspore.load_param_into_net(netG_S2T,checkpoint['netG_S2T'])
        mindspore.load_param_into_net(netG_T2S,checkpoint['netG_T2S'])
        mindspore.load_param_into_net(netD_S,checkpoint['netD_S'])
        mindspore.load_param_into_net(netD_T,checkpoint['netD_T'])
        mindspore.load_param_into_net(optimizer_G,checkpoint['optimizer_G'])
        mindspore.load_param_into_net(optimizer_D,checkpoint['optimizer_D'])
        mindspore.load_param_into_net(lr_scheduler_G,checkpoint['lr_scheduler_G'])
    ...
    if args.phase == 'test':
        transform = T.Compose([
            ...
            T.wrapper(cyclegan.transform.Translation)(netG_S2T),
        ])
...
def train(train_source_iter, train_target_iter, netG_S2T, netG_T2S, netD_S, netD_T,
    ...
    for i in range(args.iters_per_epoch):
        ...
        def forward_G_fn(real_S, real_T):
            ...
            return loss_G, fake_T, rec_S, fake_S, rec_T, loss_G_S2T, loss_G_T2S, loss_cycle_S, loss_cycle_T, identity_S, loss_identity_S, identity_T, loss_identity_T 
        
        grad_G_fn = mindspore.value_and_grad(forward_G_fn, None, optimizer_G.parameters, has_aux=True)
        
        def train_G_step(real_S, real_T):
            (loss_G, fake_T, rec_S, fake_S, rec_T, loss_G_S2T, loss_G_T2S, loss_cycle_S, loss_cycle_T, identity_S, loss_identity_S, identity_T, loss_identity_T), grads = grad_G_fn(real_S, real_T)
            optimizer_G(grads)
            return loss_G, fake_T, rec_S, fake_S, rec_T, loss_G_S2T, loss_G_T2S, loss_cycle_S, loss_cycle_T, identity_S, loss_identity_S, identity_T, loss_identity_T
        
        loss_G, fake_T, rec_S, fake_S, rec_T, loss_G_S2T, loss_G_T2S, loss_cycle_S, loss_cycle_T, identity_S, loss_identity_S, identity_T, loss_identity_T = train_G_step(real_S, real_T)
    
        # Optimize discriminator
        def forward_D_fn(fake_S,fake_T):
            set_requires_grad(netD_S, True)
            set_requires_grad(netD_T, True)
            # Calculate GAN loss for discriminator D_S
            fake_S_ = fake_S_pool.query(fake_S)
            loss_D_S = 0.5 * (criterion_gan(netD_S(real_S), True) + criterion_gan(netD_S(fake_S_), False))
            # Calculate GAN loss for discriminator D_T
            fake_T_ = fake_T_pool.query(fake_T)
            loss_D_T = 0.5 * (criterion_gan(netD_T(real_T), True) + criterion_gan(netD_T(fake_T_), False))

            return loss_D_S, loss_D_T
        
        grad_D_fn = mindspore.value_and_grad(forward_D_fn, None, optimizer_D.parameters, has_aux=True)

        def train_D_step(fake_S,fake_T):
            (loss_D_S, loss_D_T), grads = grad_D_fn(fake_S,fake_T)
            optimizer_D(grads)
            
            return loss_D_S, loss_D_T
        
        loss_D_S, loss_D_T = train_D_step(fake_S,fake_T)
