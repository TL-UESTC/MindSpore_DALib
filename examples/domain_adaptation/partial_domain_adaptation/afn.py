...
import msadapter.pytorch.backends.cudnn as cudnn
import mindspore
...
import utils
import mindspore_api.examples.domain_adaptation.partial_domain_adaptation.utils as utils
...
def main(args: argparse.Namespace):
    ...
    val_loader = GeneratorDataset(val_dataset, shuffle=False, num_parallel_workers=args.workers)
    val_loader.batch(batch_size=args.batch_size)
    test_loader = GeneratorDataset(test_dataset, shuffle=False, num_parallel_workers=args.workers)
    test_loader.batch(batch_size=args.batch_size)
    ...
    classifier = ImageClassifier(backbone, train_source_dataset.num_classes, args.num_blocks,
                                 bottleneck_dim=args.bottleneck_dim, dropout_p=args.dropout_p, pool_layer=pool_layer)
    ...
    if args.phase != 'train':
        checkpoint = mindspore.load_checkpoint(logger.get_checkpoint_path('best'))
        mindspore.load_param_into_net(classifier, checkpoint)

    # analysis the model
    if args.phase == 'analysis':
        # extract features from both domains
        feature_extractor = nn.SequentialCell(classifier.backbone, classifier.pool_layer, classifier.bottleneck)
    ...
    if args.phase == 'test':
        acc1 = utils.validate(test_loader, classifier, args)
        print(acc1)
        return
    ...
    for epoch in range(args.epochs):
        ...
        acc1 = utils.validate(val_loader, classifier, args)

        # remember best acc@1 and save checkpoint
        mindspore.save_checkpoint(classifier, logger.get_checkpoint_path('latest'))
        if acc1 > best_acc1:
    ...
    mindspore.load_param_into_net(classifier,mindspore.load_checkpoint(logger.get_checkpoint_path('best')))
    acc1 = utils.validate(test_loader, classifier, args)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
    ...
    for i in range(args.iters_per_epoch):
        ...
        # classification loss
        cls_loss = ops.cross_entropy(y_s, labels_s)
        # norm loss
