...
def get_train_transform(resizing='default', random_horizontal_flip=True, random_color_jitter=False):
    ...
    if resizing == 'default':
        ...
        transform = V.RandomResizedCrop(224)
    ...
    if random_horizontal_flip:
        transforms.append(V.RandomHorizontalFlip())
    if random_color_jitter:
        transforms.append(V.RandomColorAdjust(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5))
    transforms.extend([
        V.ToTensor(),
        V.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
